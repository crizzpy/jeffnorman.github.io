...
export const PostDelConfirmPrompt = ({key, post, delBtn, confirmPostDel, setConfirmPostDel, confirmedPostDel, setConfirmedPostDel, abortedPostDel, setAbortedPostDel}) => {
  ...
  const { posts, setPosts, 
          previewCardOpen, setPreviewCardOpen, 
          previewCardLabel, setPreviewCardLabel,
    ...
    <div className="confirm_prompt2">
        ...
        ({ item, props, key }) =>
          item && (
            <animated.div style={props}>
              <div className="del-icon" style={{ color: "white" }} id="confirm1"
                onClick={e => {
                ...
                onMouseEnter={() => {
                  ...
                  setPreviewCardLabel("Confirm")
                  let elem = document.getElementById("confirm1")
      ...
      {transition.map(
        ({ item, props, key }) =>
          item && (
            <animated.div style={props}>
              <div className="del-icon" id="undo-del" onClick={e => {
                  setConfirmPostDel(!confirmPostDel);
                  setPreviewCardOpen(false)
                }}
                onMouseEnter={() => {
                  setPreviewCardOpen(true)
                  setPreviewCardLabel("Cancel")
                  let elem = document.getElementById("undo-del")
                  let coords = elem.getBoundingClientRect()
